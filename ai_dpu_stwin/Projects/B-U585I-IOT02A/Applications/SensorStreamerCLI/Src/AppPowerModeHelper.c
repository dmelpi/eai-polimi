...
EPowerMode AppPowerModeHelper_vtblComputeNewPowerMode(IAppPowerModeHelper *_this, const SysEvent event)
{
  ...
  case SYS_PM_EVT_SRC_CTRL:
    if (power_mode == E_POWER_MODE_SENSORS_ACTIVE)
    {
      if (event.xEvent.nParam == SYS_PM_EVENT_PARAM_STOP_STREAMER)
      {
        power_mode = E_POWER_MODE_STATE1;
      }
    }
    ...
    {
      if (event.xEvent.nParam == SYS_PM_EVENT_PARAM_START_STREAMER)
      {
        power_mode = E_POWER_MODE_SENSORS_ACTIVE;
      }
    }
...
//      __HAL_RCC_PWR_CLK_ENABLE(); //TODO: STF - it doesn't exist for ST32WB.
...
#ifdef SYS_TP_MCU_STM32U5
      __HAL_PWR_CLEAR_FLAG(PWR_WAKEUP_ALL_FLAG);
#else
      __HAL_PWR_CLEAR_FLAG(PWR_FLAG_WU);
#endif /* SYS_TP_MCU_STM32U5 */
