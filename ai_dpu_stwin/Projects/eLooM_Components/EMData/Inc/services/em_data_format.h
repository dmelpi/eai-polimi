 ...
 */
sys_error_code_t EMD_Init(EMData_t *p_data, uint8_t *p_payload, uint8_t type, uint8_t mode, uint8_t dimensions, ...);
 ...
 * @param element_size [IN] specifies the size in byte of type of element defined by teh application.
 * @param mode [IN] specifies how the data value are stored / interpreted. Valid value:
 *   - E_EM_MODE_NONE
 *   - E_EM_MODE_LINEAR
 *   - E_EM_MODE_INTERLEAVED
 ...
 */
static inline
uint8_t EMD_GetType(const EMData_t *p_data);
