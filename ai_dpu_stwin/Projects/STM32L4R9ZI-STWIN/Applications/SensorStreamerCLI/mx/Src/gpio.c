...
void MX_GPIO_PE8_Init(void)
{
  ...
  HAL_NVIC_SetPriority(DHC_INT1_EXTI_IRQn, 5, 0);
  HAL_NVIC_EnableIRQ(DHC_INT1_EXTI_IRQn); //TODO: STF - I want to listen the IRQ only after the initialization when the user tasks run
}
...
void MX_GPIO_PE14_Init(void)
{
  ...
  HAL_NVIC_SetPriority(ADBW_INT1_EXTI_IRQn, 5, 0);
  HAL_NVIC_EnableIRQ(ADBW_INT1_EXTI_IRQn); //TODO: STF - I want to listen the IRQ only after the initialization when the user tasks run
...
void MX_GPIO_PF4_Init(void)
{
  ...
  HAL_NVIC_SetPriority(DHC_INT2_EXTI_IRQn, 5, 0);
  HAL_NVIC_EnableIRQ(DHC_INT2_EXTI_IRQn); //TODO: STF - I want to listen the IRQ only after the initialization when the user tasks run
}
...
void MX_GPIO_PF6_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOF, STBC02_SW_SEL_Pin, GPIO_PIN_RESET);
