...
#include "ProcesTask.h"
#include "ProcesTask_vtbl.h"
 ...
 */
static sys_error_code_t ProcesTaskExecuteStepState1(AManagedTask *_this);
 ...
 * @return SYS_NO_EROR_CODE if success, a task specific error code otherwise.
 */
static sys_error_code_t ProcesTaskExecuteStepSensorsActive(AManagedTask *_this);
 ...
 */
static void ProcesTaskDPUCallback(IDPU *_this, void* p_param);
 ...
 */
static sys_error_code_t ProcesTaskDetachFromSensor(ProcesTask_t *_this, ISourceObservable *p_sensor);
 ...
 */
static sys_error_code_t ProcesTaskSetSensorsConfig(ProcesTask_t *_this, uint16_t *p_active_sensor_id);
...
static const ProcessTaskClass_t sTheClass = {
    /* Class virtual table */
    {
        ProcesTask_vtblHardwareInit,
        ProcesTask_vtblOnCreateTask,
        ProcesTask_vtblDoEnterPowerMode,
        ProcesTask_vtblHandleError,
        ProcesTask_vtblOnEnterTaskControlLoop,
        ProcesTask_vtblForceExecuteStep,
        ProcesTask_vtblOnEnterPowerMode
    },

    /* class (PM_STATE, ExecuteStepFunc) map */
    {
        ProcesTaskExecuteStepState1,
        NULL,
        ProcesTaskExecuteStepSensorsActive,
    }
};
...
{
  return ProcesTaskAttachToSensorEx(_this, p_sensor, PRC_TASK_CFG_DATA_INPUT_USER, PRC_TASK_CFG_AXIS_NUMBER, cb_items);
}
...
sys_error_code_t ProcesTask_vtblHardwareInit(AManagedTask *_this, void *p_params)
{
  assert_param(_this != NULL);
  sys_error_code_t res = SYS_NO_ERROR_CODE;
...
sys_error_code_t ProcesTask_vtblOnCreateTask(AManagedTask *_this, TaskFunction_t *p_task_code, const char **p_name, unsigned short *p_stack_depth, void **p_params, UBaseType_t *p_priority)
{
  assert_param(_this != NULL);
  sys_error_code_t res = SYS_NO_ERROR_CODE;
  ProcesTask_t *p_obj = (ProcesTask_t*)_this;
  ...
  res = IDPU_Init((IDPU*)p_obj->p_dpu);
  if (SYS_IS_ERROR_CODE(res))
  {
    return res;
  }
  IDPU_RegisterNotifyCallback((IDPU*)p_obj->p_dpu, ProcesTaskDPUCallback, p_obj);
...
sys_error_code_t ProcesTask_vtblDoEnterPowerMode(AManagedTask *_this, const EPowerMode active_power_mode, const EPowerMode new_power_mode)
{
  assert_param(_this != NULL);
  sys_error_code_t res = SYS_NO_ERROR_CODE;
  ProcesTask_t *p_obj = (ProcesTask_t*)_this;
...
sys_error_code_t ProcesTask_vtblHandleError(AManagedTask *_this, SysEvent error)
{
  assert_param(_this != NULL);
  sys_error_code_t res = SYS_NO_ERROR_CODE;
  /*ProcesTask_t *p_obj = (ProcesTask_t*)_this;*/
...
sys_error_code_t ProcesTask_vtblOnEnterTaskControlLoop(AManagedTask *_this)
{
  assert_param(_this != NULL);
  ...
  ProcesTaskSetSensorsConfig(p_obj, &active_sensor_id);

  SYS_DEBUGF(SYS_DBG_LEVEL_VERBOSE, ("PRC: connect to sensor %d.\r\n", active_sensor_id));
  ProcesTaskAttachToSensor(p_obj, SMGetSensorObserver(active_sensor_id), 2);
...
sys_error_code_t ProcesTask_vtblForceExecuteStep(AManagedTaskEx *_this, EPowerMode active_power_mode)
{
  assert_param(_this != NULL);
  sys_error_code_t res = SYS_NO_ERROR_CODE;
  ProcesTask_t *p_obj = (ProcesTask_t*)_this;
...
sys_error_code_t ProcesTask_vtblOnEnterPowerMode(AManagedTaskEx *_this, const EPowerMode active_power_mode, const EPowerMode new_power_mode)
{
  assert_param(_this != NULL);
  sys_error_code_t res = SYS_NO_ERROR_CODE;
/*  ProcesTask_t *p_obj = (ProcesTask_t*)_this; */
    ...
    if (msg.msgId == APP_MESSAGE_ID_PRC)
    {
      switch (msg.cmd_id)
      {
        ...
        case PRC_CMD_CONNECT_TO_SENSOR:
          res = ProcesTaskAttachToSensor(p_obj, (ISourceObservable*)msg.param, msg.sparam);
          break;

        case PRC_CMD_DETACH_FROM_SENSOR:
          res = ProcesTaskDetachFromSensor(p_obj, (ISourceObservable*)msg.param);
          break;
...
{
  assert_param(_this != NULL);
  ProcesTask_t *p_obj = (ProcesTask_t*)p_param;
