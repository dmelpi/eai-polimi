...
#define EM_IS_SUPPORTED_DATA_TYPE(_dt)     (((_dt) >= E_EM_UINT8) && ((_dt) <= E_EM_FLOAT))
...
EMCompare_t EMD_Compare(const EMData_t *p_data1, const EMData_t *p_data2)
{
  ...
  /*check the basic attribute of teh data*/
...
sys_error_code_t EMD_1dInitWithCustomType(EMData_t *p_data, uint8_t *p_payload, uint16_t type, uint16_t element_size, uint16_t elemts)
{
  assert_param(p_data != NULL);
  sys_error_code_t res = SYS_NO_ERROR_CODE;
