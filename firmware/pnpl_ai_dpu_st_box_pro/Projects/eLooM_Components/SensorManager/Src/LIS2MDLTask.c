...
static const LIS2MDLTaskClass_t sTheClass =
{
    ...
    /* class::mag_sensor_if_vtbl virtual table */
    {
        ...
        LIS2MDLTask_vtblMagGetDataType,
        LIS2MDLTask_vtblMagGetDimensions,
...
sys_error_code_t LIS2MDLTask_vtblOnCreateTask(
		AManagedTask *_this,
		...
		ULONG *pParams)
{
  ...
  if(TX_SUCCESS != tx_timer_create(
		  &p_obj->read_timer,
		  "LIS2MDL_T",
		  LIS2MDLTaskTimerCallbackFunction,
		  (ULONG)TX_NULL,
                                                  ...
                                                  const EPowerMode NewPowerMode)
{
  ...
  else if(NewPowerMode == E_POWER_MODE_STATE1)
  {
    ...
    SYS_DEBUGF(SYS_DBG_LEVEL_VERBOSE, ("LIS2MDL: -> RUN\r\n"));
  }
...
{
  assert_param(_this != NULL);
  LIS2MDLTask *p_if_owner = (LIS2MDLTask*) ((uint32_t) _this - offsetof(LIS2MDLTask, mag_sensor_if));
      ...
      case SM_MESSAGE_ID_DATA_READY:
        {
          ...
          if(p_obj->pIRQConfig == NULL)
          {
            if(TX_SUCCESS != tx_timer_change(&p_obj->read_timer, AMT_MS_TO_TICKS(p_obj->lis2mdl_task_cfg_timer_period_ms), AMT_MS_TO_TICKS(p_obj->lis2mdl_task_cfg_timer_period_ms)))
            {
...
static sys_error_code_t LIS2MDLTaskSensorInit(LIS2MDLTask *_this)
{
  ...
  {
    lis2mdl_data_rate_set(p_sensor_drv, LIS2MDL_ODR_10Hz);
    _this->lis2mdl_task_cfg_timer_period_ms = 100;
  }
  else if(_this->mag_sensor_status.ODR < 21.0f)
  {
    lis2mdl_data_rate_set(p_sensor_drv, LIS2MDL_ODR_20Hz);
    _this->lis2mdl_task_cfg_timer_period_ms = 50;
  }
  else if(_this->mag_sensor_status.ODR < 51.0f)
  {
    lis2mdl_data_rate_set(p_sensor_drv, LIS2MDL_ODR_50Hz);
    _this->lis2mdl_task_cfg_timer_period_ms = 20;
  }
  else if(_this->mag_sensor_status.ODR < 101.0f)
  {
    lis2mdl_data_rate_set(p_sensor_drv, LIS2MDL_ODR_100Hz);
    _this->lis2mdl_task_cfg_timer_period_ms = 10;
  }
