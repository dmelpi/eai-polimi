...
FFT_error_t FFT_Init(FFT_instance_t *instance)
{
  ...
  /* Memory allocation */
  else if(FFT_Memory_Allocation(instance) != 0)
  {
...
{
  uint32_t x;
  for(x = 0; x < (len * ((uint32_t)ratio/2U)); x++)
  {
    dest[x] = 0.5f * (1.0f + cosf(2.0f * M_PI / ratio * (((float32_t) x / ((float32_t)len - 1.0f)) - (ratio / 2.0f))));
  }
  for(; x < (len - (len * ((uint32_t)ratio/2U))); x++)
  {
    dest[x] = 1.0f;
  }
  for(; x < len; x++)
