...
static sys_error_code_t CB_storing_int16_toInt16_helper(ADPU * _this, CBHandle_t *p_workingCB, AI_SP_Stream_t *p_stream )
{
  ...
  */
  ...
  while (nbXfer)
  {
    ...
    if (auto_transpose)
    {
      uint16_t outer_loop = n_loop/height_out;
      for (int i = 0; i < outer_loop ; i++)
      {
        for (int j = 0; j < height_out ; j++)
        {
          *(p_outData+j*width_out) = (int16_t) *p_inData++;
        }
        p_outData++;
      }
    }
...
static sys_error_code_t CB_storing_int16_toFloat_helper(ADPU * _this, CBHandle_t *p_workingCB, AI_SP_Stream_t *p_stream )
{
  ...
  */
  ...
  while (nbXfer)
  {
    ...
    {
      uint16_t outer_loop = n_loop/height_out;
      assert_param(n_loop == outer_loop * height_out);
      for (int i = 0; i < outer_loop ; i++)
      {
        for (int j = 0; j < height_out ; j++)
        {
          *(p_outData+j*width_out) = (float) *p_inData++;
        }
        p_outData++;
      }
    }
