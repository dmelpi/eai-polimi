...
IDPU *FFT_DPUAlloc() {
  IDPU *pxObj = (IDPU*) pvPortMalloc(sizeof(FFT_DPU_t));
...
sys_error_code_t FFT_DPUInit(IDPU *_this) {
  ...
  FFT_set_allocation_functions(pvPortMalloc, vPortFree);
  pObj->fft_out = NULL;
  return xRes;
}
...
uint32_t FFT_DPUSetInputParam(FFT_DPU_t *_this, uint16_t fft_size, uint8_t nAxis, uint8_t nAverage, uint8_t cb_items) {
  ...
  else
  {
    ...
    pObj->avg_counter = (uint8_t *) pvPortMalloc(sizeof (uint8_t) * nAxis);
    pObj->fft_avg = (float *) pvPortMalloc(sizeof (float) * numberOfRows * (numberOfColumns/2)); /* the output of fft is fft_size/2 */
    pObj->fft_in_copy = (float *) pvPortMalloc(sizeof (float) * numberOfRows * (numberOfColumns/2)); /* the output of fft is fft_size/2 */
  }
  /* Allocate output and buffer */
  pObj->fft_out = (float *) pvPortMalloc(sizeof (float) * numberOfRows * (numberOfColumns/2)); /* the output of fft is fft_size/2 */
  pObj->super.dpuOutStream.packet.payload = (uint8_t *)pObj->fft_out;  
  
  /* init fft library */
  pObj->fft_instance.init_params.FFT_len = numberOfColumns;
  pObj->fft_instance.init_params.overlap = 0.0f;
