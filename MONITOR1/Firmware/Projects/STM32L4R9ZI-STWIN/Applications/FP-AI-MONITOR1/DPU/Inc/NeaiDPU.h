...
typedef enum _ENeaiMode {
  ...
  E_NEAI_LEARNING,     //!< the AI library learn from the signals.
  E_NEAI_DETECTION     //!< the AI library detect the similarity of the signal with the learned model.
} ENeaiMode_t;
...
struct _NeaiDPU_t {
   ...
   * - NanoEdgeAI_learn()
   */
  uint8_t (*neai_processing_f)(float data_input[]);
};
...
ENeaiMode_t NeaiDPUGetProcessingMode(NeaiDPU_t *_this)
{
  ...
  ENeaiMode_t res = E_NEAI_MODE_NONE;

  if (_this->neai_processing_f == NanoEdgeAI_learn)
  {
    res = E_NEAI_LEARNING;
  }
  else if (_this->neai_processing_f == NanoEdgeAI_detect)
  {
    res = E_NEAI_DETECTION;
  }
