                                       ...
                                       AI_SP_SpectralFlatnessOut_t *pOutput)
{
  ...
  for (uint32_t outer_idx = 0; outer_idx < outer_data_loop; outer_idx++)
  {
      ...
      mean = 0.0f;
      mean_log = 0.0f;
      pDataIn = (const float32_t *)packetIn.payload + outer_idx*outer_data_stride + b1*inner_data_stride;
      for (i = b1; i <= b2; i++)
      {
        mean_log += logf(*pDataIn + AI_SP_EPSILON);
        mean += *pDataIn;
                                   ...
                                   AI_SP_SpectralPeakOut_t *pOutput)
{
  ...
  for (uint32_t outer_idx = 0; outer_idx < outer_data_loop; outer_idx++)
  {
    ...
    if (peak_power_idx == 0)
    {
      noise_power = power_sum - peak_power - pDataIn[(peak_power_idx + 1)*inner_data_stride];
      noise_power /= (n_freq - 2);
    }
    else if (peak_power_idx == (inner_data_loop - 1))
    {
      noise_power = power_sum - pDataIn[(peak_power_idx - 1)*inner_data_stride] - peak_power;
      noise_power /= (n_freq - 2);
    }
    else
    {
      noise_power = power_sum - pDataIn[(peak_power_idx - 1)*inner_data_stride] - peak_power - pDataIn[(peak_power_idx + 1)*inner_data_stride];
