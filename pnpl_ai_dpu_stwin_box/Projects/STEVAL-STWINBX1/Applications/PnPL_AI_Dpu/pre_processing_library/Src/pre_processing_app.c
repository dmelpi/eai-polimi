...
void pre_processing_process(tridimensional_data_t* data_in, uint32_t data_in_size, float32_t* data_out, uint32_t data_out_size, pre_processing_data_t* pre_processing_data) {
    ...
    */
    ...
    //float32_t data_1[data_in_size];
    //signal_normalization(data_0, data_in_size, data_1, data_in_size, 2, 0);

    // FFT
    fft(data_0, data_in_size, data_out, data_out_size, &((*pre_processing_data).mfcc_handler), (*pre_processing_data).mfcc_signal_windowing, (*pre_processing_data).mfcc_multipliers);
