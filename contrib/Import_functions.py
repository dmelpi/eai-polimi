...
def load_data(resampling_active, Resampling_frequency):
    # load acquired data
...
def CNN_time_series_sorting_input_matrix(DataFrame,sliding_samples,window_length):
    
    N=0
    while (len(DataFrame)-(N*sliding_samples+window_length)) > 0:
        N=N+1
    
    X_class=np.ones((N,int(window_length),3))
    for ii in tqdm(range(N)):
        df_acc=DataFrame.iloc[ii*sliding_samples:ii*sliding_samples+window_length]
        
        X_support=np.array(df_acc.iloc[:,1:4])
        X_support=gravity_rotation(X_support)
        
        X_class[ii,:,0]=X_support[:,0]#-np.mean(X_support[:,0])
        X_class[ii,:,1]=X_support[:,1]#-np.mean(X_support[:,1])
        X_class[ii,:,2]=X_support[:,2]#-np.mean(X_support[:,2])
...
def time_series_features_sorting_input_matrix(DataFrame,sliding_samples,window_length):
    ...
    X_class_t = pd.DataFrame(columns=['Max_x','Max_y','Max_z',
                                      'Min_x','Min_y','Min_z',
                                      'logSumExp_x','logSumExp_y','logSumExp_z',
                                      'RMS_x','RMS_y','RMS_z',
                                      'Std_x','Std_y','Std_z'])
    
    X_class_f = pd.DataFrame(columns=['Max_f_x','Max_f_y','Max_f_z',
                                      'Argmax_f_x','Argmax_f_y','Argmax_f_z',
                                      'Sum_f_x','Sum_f_y','Sum_f_z',
                                      'logSumExp_f_x','logSumExp_f_y','logSumExp_f_z',
                                      'RMS_f_x','RMS_f_y','RMS_f_z'
                                      ])
    
    
    window_hann = np.hanning(window_length)
    X_class=np.ones((N,int(window_length),3))
    for ii in tqdm(range(N)):
        df_acc=DataFrame.iloc[ii*sliding_samples:ii*sliding_samples+window_length]
        ...
        time_series_df = extract_time_series_features(X_support)
        X_class_t = X_class_t.append(time_series_df)
        
        
        X_support[:,0]=X_support[:,0]*window_hann
        X_support[:,1]=X_support[:,1]*window_hann
        X_support[:,2]=X_support[:,2]*window_hann

        FFT_0=np.abs(fft(X_support[:,0]))/len(X_support[:,0])
        FFT_Halves_0=2*FFT_0[0:int(window_length/2)]
        FFT_Halves_0[0]=FFT_Halves_0[0]/2
        
        FFT_1=np.abs(fft(X_support[:,1]))/len(X_support[:,1])
        FFT_Halves_1=2*FFT_1[0:int(window_length/2)]
        FFT_Halves_1[0]=FFT_Halves_1[0]/2
        
        FFT_2=np.abs(fft(X_support[:,2]))/len(X_support[:,2])
        FFT_Halves_2=2*FFT_2[0:int(window_length/2)]
        FFT_Halves_2[0]=FFT_Halves_2[0]/2
        
        FFT = np.zeros((len(FFT_Halves_0),3))
        FFT[:,0] = FFT_Halves_0
        FFT[:,1] = FFT_Halves_1
        FFT[:,2] = FFT_Halves_2
        
        
        frequency_domain_df = extract_frequency_domain_features(FFT)
        
        X_class_f = X_class_f.append(frequency_domain_df)
        
        X_class = pd.concat([X_class_t,X_class_f],axis=1)
...
def extract_time_series_features(X):
    df_features=np.zeros((1,5*3))
    
    df_features[0,0]=max(X[:,0])
    df_features[0,1]=max(X[:,1])
    df_features[0,2]=max(X[:,2])
    df_features[0,3]=min(X[:,0])
    df_features[0,4]=min(X[:,1])
    df_features[0,5]=min(X[:,2])
    df_features[0,6]=np.log(np.sum(np.exp(X[:,0])))
    df_features[0,7]=np.log(np.sum(np.exp(X[:,1])))
    df_features[0,8]=np.log(np.sum(np.exp(X[:,2])))
    df_features[0,9]=np.sqrt(sum(X[:,0]**2)/len(X[:,0]))
    df_features[0,10]=np.sqrt(sum(X[:,1]**2)/len(X[:,1]))
    df_features[0,11]=np.sqrt(sum(X[:,2]**2)/len(X[:,2]))
    df_features[0,12]=np.std(X[:,0])
    df_features[0,13]=np.std(X[:,1])
    df_features[0,14]=np.std(X[:,2])
    
    df_features = pd.DataFrame(df_features, columns=['Max_x','Max_y','Max_z',
                                      'Min_x','Min_y','Min_z',
                                      'logSumExp_x','logSumExp_y','logSumExp_z',
                                      'RMS_x','RMS_y','RMS_z',
                                      'Std_x','Std_y','Std_z'])
...
def extract_frequency_domain_features(X):
    ...
    df_features[0,1]=max(X[:,1])
    df_features[0,2]=max(X[:,2])
    df_features[0,3]=np.argmax(X[:,0])
    df_features[0,4]=np.argmax(X[:,1])
    df_features[0,5]=np.argmax(X[:,2])
    df_features[0,6]=sum(X[:,0])
    df_features[0,7]=sum(X[:,1])
    df_features[0,8]=sum(X[:,2])
    df_features[0,9]=np.log(np.sum(np.exp(X[:,0])))
    df_features[0,10]=np.log(np.sum(np.exp(X[:,1])))
    df_features[0,11]=np.log(np.sum(np.exp(X[:,2])))
    df_features[0,12]=np.sqrt(sum(X[:,0]**2)/len(X[:,0]))
    df_features[0,13]=np.sqrt(sum(X[:,1]**2)/len(X[:,1]))
    df_features[0,14]=np.sqrt(sum(X[:,2]**2)/len(X[:,2]))
    
    df_features = pd.DataFrame(df_features, columns=['Max_f_x','Max_f_y','Max_f_z',
                                      'Argmax_f_x','Argmax_f_y','Argmax_f_z',
                                      'Sum_f_x','Sum_f_y','Sum_f_z',
                                      'logSumExp_f_x','logSumExp_f_y','logSumExp_f_z',
                                      'RMS_f_x','RMS_f_y','RMS_f_z'
...
def evaluate_model(trainX, trainy, validationX, validationy, testX, testy , model, learningRate, batchSize=128, verbosity=0, learning_curves=False, ConfusionMatrix=False ):
...
def signal_spectra(DataFrame, sensor_Type):
    ...
    delta_f=1/T
    freq=np.arange(0,sampling_f/2,delta_f)
    FFT=np.abs(fft(Data))/len(Data)
    FFT_Halves=2*FFT[0:len(freq)]
    FFT_Halves[0]=FFT_Halves[0]/2
    return freq,FFT_Halves
