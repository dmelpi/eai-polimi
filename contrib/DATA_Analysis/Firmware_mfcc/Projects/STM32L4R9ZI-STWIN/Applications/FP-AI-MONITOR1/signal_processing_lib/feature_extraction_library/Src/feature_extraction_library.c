...
float32_t dot_product(float32_t* in_1, float32_t* in_2){
	...
	float32_t multOutput[input_vector_len/2];
	float32_t dot_out=0;
	arm_mult_f32(in_1, in_2, multOutput, input_vector_len/2);
	for (int i=0;  i<input_vector_len/2;   i++){
...
void signal_normalization(float32_t *dataIn, float32_t * dataOut){
	...
	arm_max_f32(dataIn,input_vector_len,&max,&maxpos);
	arm_min_f32(dataIn,input_vector_len,&min,&minpos);

	for(int i=0; i<input_vector_len ; i++){
...
void DoHanning(float32_t *dataIn, float32_t * dataOut ){
	for (int i = 0; i < input_vector_len; i++) {
		float32_t multiplier = 0.5 * (1 - arm_cos_f32((float32_t) 2*PI*i/(input_vector_len-1)));
...
void DoFFT(arm_rfft_fast_instance_f32 * fft_handler, float32_t *fft_in_buf ,float32_t * spectrum){
	...
	static float32_t fft_out_buf[input_vector_len];
	arm_rfft_fast_f32(fft_handler, (float32_t*)  fft_in_buf, (float32_t*) fft_out_buf, 0);

	int freqpoint = 0;

	for (int i=0;   i<input_vector_len   ; i=i+2) {
	  spectrum[freqpoint] =(complexABS(fft_out_buf[i], fft_out_buf[i+1]))/(sqrt(input_vector_len));
	  spectrum[freqpoint] = 2 * spectrum[freqpoint]*spectrum[freqpoint] /(float32_t)sample_rate;
...
void mel_spectrum(int * bin , float32_t * power_spectrum, float32_t * mel_spectra ){
	...
	static float32_t in_vector[input_vector_len/2];
