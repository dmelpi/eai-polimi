...
static const ISensor_vtbl sIIS3DWBTask_AccSensorIF_vtbl = {
 ...
 *
 * @return IIS3DWBTask task object.
 */
static IIS3DWBTask * IIS3DWBTaskGetInstance(void);
...
AManagedTaskEx *IIS3DWBTaskAlloc() {
  ...
  sTaskObj.acc_sensor_if.vptr = &sIIS3DWBTask_AccSensorIF_vtbl;
...
uint8_t IIS3DWBTask_vtblAccGetId(ISourceObservable *_this){
  assert_param(_this);
  IIS3DWBTask *p_iis3dwb_instance = IIS3DWBTaskGetInstance();
  uint8_t res = p_iis3dwb_instance->acc_id;
...
IEventSrc *IIS3DWBTask_vtblGetEventSourceIF(ISourceObservable *_this){
  assert_param(_this);
  IIS3DWBTask *p_iis3dwb_instance = IIS3DWBTaskGetInstance();
  return p_iis3dwb_instance->p_event_src;
}
...
float IIS3DWBTask_vtblAccGetFS(ISourceObservable *_this){
  assert_param(_this);
  IIS3DWBTask *p_iis3dwb_instance = IIS3DWBTaskGetInstance();
  float res = p_iis3dwb_instance->sensor_status.pSubSensorStatus[0].FS;
...
float IIS3DWBTask_vtblAccGetSensitivity(ISourceObservable *_this){
  assert_param(_this);
  IIS3DWBTask *p_iis3dwb_instance = IIS3DWBTaskGetInstance();
  float res = p_iis3dwb_instance->sensor_status.pSubSensorStatus[0].Sensitivity;
...
sys_error_code_t IIS3DWBTask_vtblSensorSetODR(ISensor_t *_this, float ODR){
  ...
  IIS3DWBTask *p_iis3dwb_instance = IIS3DWBTaskGetInstance();
  EPowerMode log_status = AMTGetTaskPowerMode((AManagedTask *)p_iis3dwb_instance);
  ...
  else
  {
    ...
    res = IIS3DWBTaskPostReportToBack(p_iis3dwb_instance, (SMMessage*)&report);
  }
...
boolean_t IIS3DWBTask_vtblSensorIsEnabled(ISensor_t *_this){
  ...
  if(ISourceGetId((ISourceObservable *)_this) == p_iis3dwb_instance->acc_id)
    res = p_iis3dwb_instance->sensor_status.pSubSensorStatus[0].IsActive;
...
SensorDescriptor_t IIS3DWBTask_vtblSensorGetDescription(ISensor_t *_this){
  ...
  IIS3DWBTask *p_iis3dwb_instance = IIS3DWBTaskGetInstance();
  return p_iis3dwb_instance->sensor_descriptor;
}
...
static IIS3DWBTask* IIS3DWBTaskGetInstance(){
  return (IIS3DWBTask*)&sTaskObj;
}
...
static sys_error_code_t IIS3DWBTaskSensorInit(IIS3DWBTask *_this) {
  ...
  iis3dwb_wtm_level = ((uint16_t)_this->sensor_status.pSubSensorStatus[0].ODR * (uint16_t)IIS3DWB_MAX_DRDY_PERIOD);
  ...
  iis3dwb_xl_hp_path_on_out_set(p_sensor_drv,IIS3DWB_LP_6k3Hz);
...
static sys_error_code_t IIS3DWBTaskSensorReadData(IIS3DWBTask *_this) {
  ...
  iis3dwb_read_reg(p_sensor_drv, IIS3DWB_FIFO_STATUS1, &reg0, 1);
  iis3dwb_read_reg(p_sensor_drv, IIS3DWB_FIFO_STATUS2, &reg1, 1);
...
static sys_error_code_t IIS3DWBTaskSensorInitTaskParams(IIS3DWBTask *_this) {
  ...
  /* SENSOR DESCRIPTOR */
  strcpy(_this->sensor_descriptor.Name, "IIS3DWB");
  ...
  _this->sensor_descriptor.pSubSensorDescriptor[0].pODR[0] = 26667.0f;
  ...
  _this->sensor_status.pSubSensorStatus[0].ODR = 26667.0f;
