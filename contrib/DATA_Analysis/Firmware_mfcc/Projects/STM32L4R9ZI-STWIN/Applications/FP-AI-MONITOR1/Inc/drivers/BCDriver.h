 ...
 ******************************************************************************
 */
#ifndef INC_DRIVERS_BCDRIVER_H_
#define INC_DRIVERS_BCDRIVER_H_
 ...
 */
typedef struct _BCDriverParams_t BCDriverParams_t;

/**
 * Create  type name for _BCDriver_t.
 */
typedef struct _BCDriver_t BCDriver_t;

/**
 * Initialization parameters for the driver.
 */
struct _BCDriverParams_t
{
  /**
...
/**
 * Register the tick callback with the driver. The callback is called every time
 * the driver time base tick is elapsed.
 *
...
/**
 * Toggle the PIN used to implement the 1-wire communication channel.
 *
 * @param _this [IN] specifies a pointer to a driver object.
 * @return SYS_NO_ERROR_CODE
 */
inline sys_error_code_t BCDriverToggleLine1W(BCDriver_t *_this);

/**
 * Set low the PIN used to implement the 1-wire communication channel.
 *
 * @param _this [IN] specifies a pointer to a driver object.
 * @return SYS_NO_ERROR_CODE
 */
inline sys_error_code_t BCDriverResetLine1W(BCDriver_t *_this);
...
SYS_DEFINE_INLINE
sys_error_code_t BCDriverToggleLine1W(BCDriver_t *_this)
{
  assert_param(_this != NULL);

  HAL_GPIO_TogglePin(_this->mx_handle.p_mx_gpio_cfg->port, _this->mx_handle.p_mx_gpio_cfg->pin);

  return SYS_NO_ERROR_CODE;
}

SYS_DEFINE_INLINE
sys_error_code_t BCDriverResetLine1W(BCDriver_t *_this)
{
  assert_param(_this != NULL);

  HAL_GPIO_WritePin(_this->mx_handle.p_mx_gpio_cfg->port, _this->mx_handle.p_mx_gpio_cfg->pin, GPIO_PIN_RESET);
