...
uint8_t FwInfoPnPL_vtblGetStatus(IPnPLComponent_t *_this, char **serializedJSON, uint32_t *size, uint8_t pretty)
{
  ...
  fwinfo_get_part_number(&temp);
  json_object_dotset_string(JSON_Status, "fwinfo.part_number", temp);
  fwinfo_get_device_url(&temp);
  json_object_dotset_string(JSON_Status, "fwinfo.device_url", temp);
  fwinfo_get_data_format(&temp);
  json_object_dotset_string(JSON_Status, "fwinfo.data_format", temp);
  fwinfo_get_data_ext(&temp);
  json_object_dotset_string(JSON_Status, "fwinfo.data_ext", temp);
