...
AManagedTaskEx *HTS221TaskAlloc(const void *pIRQConfig, const void *pCSConfig, EBusType eBusType)
{
		...
		ULONG *pParams)
{
  ...
  if(TX_SUCCESS != tx_queue_create(&p_obj->in_queue, "HTS221_Q", nItemSize / 4, pvQueueItemsBuff, HTS221_TASK_CFG_IN_QUEUE_LENGTH * nItemSize))
  {
    res = SYS_TASK_HEAP_OUT_OF_MEMORY_ERROR_CODE;
    SYS_SET_SERVICE_LEVEL_ERROR_CODE(res);
    return res;
...
//      SYS_DEBUGF(SYS_DBG_LEVEL_VERBOSE, ("HTS221: new data.\r\n"));
          if(p_obj->pIRQConfig == NULL)
          {
            if(TX_SUCCESS != tx_timer_change(&p_obj->read_timer, AMT_MS_TO_TICKS(HTS221_TASK_CFG_TIMER_PERIOD_MS), AMT_MS_TO_TICKS(HTS221_TASK_CFG_TIMER_PERIOD_MS)))
            {
