...
extern sys_error_code_t MDFDriverFilterRegisterCallback(MDFDriver_t *_this, HAL_MDF_CallbackIDTypeDef CallbackID, pMDF_CallbackTypeDef pCallback);
#endif
...
sys_error_code_t MDFDriver_vtblDoEnterPowerMode(IDriver *_this, const EPowerMode active_power_mode, const EPowerMode new_power_mode)
{
  assert_param(_this != NULL);
  sys_error_code_t res = SYS_NO_ERROR_CODE;
  /*  MDFDriver_t *p_obj = (MDFDriver_t*)_this; */
