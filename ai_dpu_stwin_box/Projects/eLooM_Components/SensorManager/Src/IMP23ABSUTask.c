...
struct _IMP23ABSUTask
{
   ...
   */
  const SensorDescriptor_t *mic_sensor_descriptor;
   ...
   * Buffer to store the data read from the sensor
   */
  int16_t p_dummy_data_buff[((MAX_AMIC_SAMPLING_FREQUENCY / 1000) * 2)];
#endif
...
#if (HSD_USE_DUMMY_DATA == 1)
/**
 * Read fake data from the sensor.
 *
 * @param _this [IN] specifies a pointer to a task object.
 * @return SYS_NO_EROR_CODE if success, a task specific error code otherwise.
 */
static sys_error_code_t IMP23ABSUTaskSensorReadFakeData(IMP23ABSUTask *_this);
#endif
...
static const IMP23ABSUTaskClass_t sTheClass =
{
    ...
    /* class::mic_sensor_if_vtbl virtual table */
    {
		...
		IMP23ABSUTask_vtblMicGetDataType,
		IMP23ABSUTask_vtblMicGetDimensions,
...
AManagedTaskEx* IMP23ABSUTaskAlloc(const void *p_mx_mdf_cfg, const void *p_mx_adc_cfg)
{
  ...
  sTaskObj.mic_sensor_descriptor = &sTheClass.mic_class_descriptor;
...
sys_error_code_t IMP23ABSUTask_vtblOnCreateTask(
		AManagedTask *_this,
		...
		ULONG *pParams)
{
  ...
  *pAutoStart = TX_AUTO_START;
  ...
  uint16_t nItemSize = IMP23ABSU_TASK_CFG_IN_QUEUE_ITEM_SIZE;
  VOID *pvQueueItemsBuff = SysAlloc(IMP23ABSU_TASK_CFG_IN_QUEUE_LENGTH * nItemSize);
  ...
  if(TX_SUCCESS != tx_queue_create(&p_obj->in_queue, "IMP23ABSU_Q", nItemSize / 4, pvQueueItemsBuff, IMP23ABSU_TASK_CFG_IN_QUEUE_LENGTH * nItemSize))
  {
    res = SYS_TASK_HEAP_OUT_OF_MEMORY_ERROR_CODE;
    SYS_SET_SERVICE_LEVEL_ERROR_CODE(res);
    return res;
...
sys_error_code_t IMP23ABSUTask_vtblDoEnterPowerMode(AManagedTask *_this, const EPowerMode ActivePowerMode, const EPowerMode NewPowerMode)
{
  ...
  else if(NewPowerMode == E_POWER_MODE_STATE1)
  {
    ...
    SYS_DEBUGF(SYS_DBG_LEVEL_VERBOSE, ("IMP23ABSU: -> RUN\r\n"));
  }
...
SensorDescriptor_t IMP23ABSUTask_vtblSensorGetDescription(ISensor_t *_this)
{
  ...
  return *p_if_owner->mic_sensor_descriptor;
}
...
#if (HSD_USE_DUMMY_DATA == 1)
        res = IMP23ABSUTaskSensorReadFakeData(p_obj);
#endif
