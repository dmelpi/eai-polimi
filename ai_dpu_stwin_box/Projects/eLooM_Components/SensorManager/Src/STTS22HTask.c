...
AManagedTaskEx* STTS22HTaskAlloc(const void *pIRQConfig, const void *pCSConfig, EBusType eBusType)
{
		...
		ULONG *pParams)
{
  ...
  if(TX_SUCCESS != tx_queue_create(&p_obj->in_queue, "STTS22H_Q", nItemSize / 4, pvQueueItemsBuff, STTS22H_TASK_CFG_IN_QUEUE_LENGTH * nItemSize))
  {
    res = SYS_TASK_HEAP_OUT_OF_MEMORY_ERROR_CODE;
    SYS_SET_SERVICE_LEVEL_ERROR_CODE(res);
    return res;
...
sys_error_code_t STTS22HTask_vtblDoEnterPowerMode(AManagedTask *_this, const EPowerMode ActivePowerMode, const EPowerMode NewPowerMode)
{
  assert_param(_this);
  sys_error_code_t res = SYS_NO_ERROR_CODE;
  STTS22HTask *p_obj = (STTS22HTask*) _this;
...
sys_error_code_t STTS22HTask_vtblOnEnterPowerMode(AManagedTaskEx *_this, const EPowerMode ActivePowerMode, const EPowerMode NewPowerMode)
{
  assert_param(_this);
  sys_error_code_t res = SYS_NO_ERROR_CODE;
  //  STTS22HTask *p_obj = (STTS22HTask*)_this;
