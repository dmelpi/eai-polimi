...
#define IMP34DT05_TASK_CFG_IN_QUEUE_LENGTH      100
#endif
...
struct _IMP34DT05Task
{
   ...
   * Buffer to store the data read from the sensor
   */
  int16_t p_dummy_data_buff[((MAX_DMIC_SAMPLING_FREQUENCY / 1000) * 2)];
#endif
  /*
...
typedef struct _IMP34DT05TaskClass
{
   ...
   */
  ISensor_vtbl mic_sensor_if_vtbl;

  /**
   * Specifies mic sensor capabilities.
   */
  SensorDescriptor_t mic_class_descriptor;
 ...
 * @return SYS_NO_EROR_CODE if success, a task specific error code otherwise.
 */
static sys_error_code_t IMP34DT05TaskSensorReadFakeData(IMP34DT05Task *_this);
#endif
...
static const IMP34DT05TaskClass_t sTheClass =
{
    ...
    /* class::mic_sensor_if_vtbl virtual table */
    {
		...
		IMP34DT05Task_vtblMicGetDataType,
		IMP34DT05Task_vtblMicGetDimensions,
...
AManagedTaskEx* IMP34DT05TaskAlloc(const void *p_mx_mdf_cfg)
{
  ...
  sTaskObj.mic_sensor_if.vptr = &sTheClass.mic_sensor_if_vtbl;
  sTaskObj.sensor_descriptor = &sTheClass.mic_class_descriptor;
                                                ...
                                                ULONG *pParams)
{
  ...
  *pAutoStart = TX_AUTO_START;
  ...
  uint16_t nItemSize = IMP34DT05_TASK_CFG_IN_QUEUE_ITEM_SIZE;
  VOID *pvQueueItemsBuff = SysAlloc(IMP34DT05_TASK_CFG_IN_QUEUE_LENGTH * nItemSize);
  ...
  if(TX_SUCCESS != tx_queue_create(&p_obj->in_queue, "IMP34DT05_Q", nItemSize / 4, pvQueueItemsBuff, IMP34DT05_TASK_CFG_IN_QUEUE_LENGTH * nItemSize))
  {
    res = SYS_TASK_HEAP_OUT_OF_MEMORY_ERROR_CODE;
    SYS_SET_SERVICE_LEVEL_ERROR_CODE(res);
    return res;
...
sys_error_code_t IMP34DT05Task_vtblDoEnterPowerMode(AManagedTask *_this, const EPowerMode ActivePowerMode, const EPowerMode NewPowerMode)
{
  ...
  else if(NewPowerMode == E_POWER_MODE_STATE1)
  {
    ...
    SYS_DEBUGF(SYS_DBG_LEVEL_VERBOSE, ("IMP34DT05: -> RUN\r\n"));
  }
...
{
  assert_param(_this);
  IMP34DT05Task *p_if_owner = (IMP34DT05Task*) ((uint32_t) _this - offsetof(IMP34DT05Task, mic_sensor_if));
...
sys_error_code_t IMP34DT05Task_vtblSensorSetODR(ISensor_t *_this, float ODR)
{
  ...
  else
  {
    ...
    res = IMP34DT05TaskPostReportToFront(p_if_owner, (SMMessage*) &report);
  }
...
//          SYS_DEBUGF(SYS_DBG_LEVEL_VERBOSE, ("IMP34DT05: new data.\r\n"));
...
#if (HSD_USE_DUMMY_DATA == 1)
          res = IMP34DT05TaskSensorReadFakeData(p_obj);
#endif
          ...
          if(!SYS_IS_ERROR_CODE(res))
          {
            ...
            {
                .packet.payload = (uint8_t*) &p_obj->p_dummy_data_buff[(p_obj->half - 1) * (((uint32_t)p_obj->sensor_status.ODR / 1000) )],
            ...
            AI_SP_Stream_t stream =
            {
                .packet.payload = (uint8_t*) &p_obj->p_sensor_data_buff[(p_obj->half - 1) * (((uint32_t) p_obj->sensor_status.ODR / 1000))],
                .packet.payload_fmt = AI_SP_FMT_INT16_RESET(),
                .mode = AI_SP_MODE_FULL
            // bonus question: what is AI_LOGGING_SHAPES_DEPTH ??
            // (can I represent anomogeneous matrix [4*4] with this data format ?
...
#endif
            ai_logging_create_shape_1d(&stream.packet.shape, ((p_obj->sensor_status.ODR / 1000)));
            ...
            /* update measuredODR */
            p_obj->sensor_status.MeasuredODR = p_obj->sensor_status.ODR;
...
static sys_error_code_t IMP34DT05TaskSensorReadFakeData(IMP34DT05Task *_this)
{
  ...
  void *p_data_ptr = &_this->p_dummy_data_buff[(_this->half - 1) * (((uint32_t)_this->sensor_status.ODR / 1000) )]; /* void since it is independent from data format*/
  //  /* Do something */
  uint16_t idx = 0;
...
static sys_error_code_t IMP34DT05TaskSensorInitTaskParams(IMP34DT05Task *_this)
{
  ...
  _this->sensor_status.DataPacketSize = 4096;

#if (HSD_USE_DUMMY_DATA == 1)
  _this->sensor_status.SamplesPerTimestamp = 0;
#else
  _this->sensor_status.SamplesPerTimestamp = 1000;
#endif
...
void MDF_Filter_0_Complete_Callback(MDF_HandleTypeDef *hmdf)
{
  sTaskObj.timestamp = SMUtilGetTimeStamp();

  SMMessage report =
  {
      .sensorDataReadyMessage.messageId = SM_MESSAGE_ID_DATA_READY,
      .sensorDataReadyMessage.half = 2,
