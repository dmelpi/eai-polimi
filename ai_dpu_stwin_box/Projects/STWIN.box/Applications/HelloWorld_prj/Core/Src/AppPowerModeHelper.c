...
sys_error_code_t AppPowerModeHelper_vtblInit(IAppPowerModeHelper *_this)
{
  ...
  p_obj->status.active_ai_lib_id = SYS_NEAI_ID;
...
EPowerMode AppPowerModeHelper_vtblComputeNewPowerMode(IAppPowerModeHelper *_this, const SysEvent event)
{
  ...
  case SYS_PM_EVT_SRC_PB:
    ...
    {
      /* new state depends on what processing is active */
      if (p_obj->status.active_ai_lib_id == SYS_X_CUBE_AI_ID)
      {
        power_mode = E_POWER_MODE_X_CUBE_AI_ACTIVE;
      }
      else if (p_obj->status.active_ai_lib_id == SYS_NEAI_ID)
      {
  ...
  case SYS_PM_EVT_SRC_CTRL:
    if (power_mode == E_POWER_MODE_STATE1)
    {
      ...
      else if (event.xEvent.nParam == SYS_PM_EVENT_PARAM_START_NEAI)
      {
        power_mode = E_POWER_MODE_NEAI_ACTIVE;
        p_obj->status.active_ai_lib_id = SYS_NEAI_ID;
      }
