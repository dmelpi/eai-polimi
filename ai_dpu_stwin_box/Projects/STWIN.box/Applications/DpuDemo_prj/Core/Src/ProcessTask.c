...
#ifndef PRC_TASK_CFG_IN_QUEUE_ITEM_COUNT
#define PRC_TASK_CFG_IN_QUEUE_ITEM_COUNT    10
#endif
    ...
    ULONG *pParams)
{
  ...
  VOID *pvQueueItemsBuff = SysAlloc(PRC_TASK_CFG_IN_QUEUE_ITEM_COUNT * item_size);
  ...
  if(TX_SUCCESS != tx_queue_create(&p_obj->in_queue, "ProcTask_Q", item_size/4, pvQueueItemsBuff, PRC_TASK_CFG_IN_QUEUE_ITEM_COUNT * item_size))
  {
    res = SYS_TASK_HEAP_OUT_OF_MEMORY_ERROR_CODE;
    SYS_SET_SERVICE_LEVEL_ERROR_CODE(res);
    sys_error_handler();
