...
static int32_t BSP_HTS221_Init(void);
static int32_t BSP_HTS221_DeInit(void);
#endif
...
static int32_t HTS221_0_Probe(uint32_t Functions)
{
  ...
  else
  {
    ...
    EnvCtx[HTS221_0].Functions = ((uint32_t) cap.Temperature) | ((uint32_t) cap.Pressure << 1) | ((uint32_t) cap.Humidity << 2);
...
static int32_t BSP_HTS221_Init(void)
{
  ...
  BSP_HTS221_INT_GPIO_CLK_ENABLE();

  /*Configure GPIO pin : PC1 */
  GPIO_InitStruct.Pin = BSP_HTS221_INT_PIN;
  GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(BSP_HTS221_INT_PORT, &GPIO_InitStruct);

  /* EXTI interrupt init*/
  /* Enable and set EXTI Interrupt priority */
  HAL_NVIC_SetPriority(BSP_HTS221_INT_EXTI_IRQn, BSP_HTS221_INT_EXTI_IRQ_PP, BSP_HTS221_INT_EXTI_IRQ_SP);
  HAL_NVIC_EnableIRQ(BSP_HTS221_INT_EXTI_IRQn);
