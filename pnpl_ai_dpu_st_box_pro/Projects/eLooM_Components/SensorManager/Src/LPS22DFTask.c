...
static const LPS22DFTaskClass_t sTheClass =
{
    ...
    /* class::press_sensor_if_vtbl virtual table */
    {
        ...
        LPS22DFTask_vtblPressGetDataType,
        LPS22DFTask_vtblPressGetDimensions,
...
sys_error_code_t LPS22DFTask_vtblOnCreateTask(
		AManagedTask *_this,
		...
		ULONG *pParams)
{
  ...
  if(TX_SUCCESS != tx_timer_create(
		  ...
		  "ILPS22WS_T",
		  LPS22DFTaskTimerCallbackFunction,
		  (ULONG)TX_NULL,
		  AMT_MS_TO_TICKS(LPS22DF_TASK_CFG_TIMER_PERIOD_MS),
		  0,
		  TX_NO_ACTIVATE))
  {
    res = SYS_TASK_HEAP_OUT_OF_MEMORY_ERROR_CODE;
                                                   ...
                                                   const EPowerMode NewPowerMode)
{
  ...
  else if(NewPowerMode == E_POWER_MODE_STATE1)
  {
    ...
    SYS_DEBUGF(SYS_DBG_LEVEL_VERBOSE, ("LPS22DF: -> RUN\r\n"));
  }
...
{
  assert_param(_this != NULL);
  LPS22DFTask *p_if_owner = (LPS22DFTask*) ((uint32_t) _this - offsetof(LPS22DFTask, press_sensor_if));
...
//          SYS_DEBUGF(SYS_DBG_LEVEL_VERBOSE, ("LPS22DF: new data.\r\n"));
          ...
          if(!SYS_IS_ERROR_CODE(res))
          {
            ...
            {
            EMData_t data;
            EMD_Init(&data, (uint8_t*)&p_obj->p_press_data_buff[0], E_EM_FLOAT, E_EM_MODE_LINEAR, 1, p_obj->samples_per_it);

            DataEvent_t evt;

            DataEventInit((IEvent*)&evt, p_obj->p_press_event_src, &data, timestamp, p_obj->press_id);
...
//          SYS_DEBUGF(SYS_DBG_LEVEL_VERBOSE, ("LPS22DF: ts = %f\r\n", (float)timestamp));
            if(p_obj->pIRQConfig == NULL)
            {
...
static sys_error_code_t LPS22DFTaskSensorInit(LPS22DFTask *_this)
{
  ...
  float lps22df_odr;
  lps22df_odr = _this->press_sensor_status.ODR;
  if(lps22df_odr < 2.0f)
  {
    md.odr = LPS22DF_1Hz;
...
static sys_error_code_t LPS22DFTaskSensorInitTaskParams(LPS22DFTask *_this)
{
  ...
  /* PRESSURE STATUS */
  _this->press_sensor_status.DataType = E_EM_FLOAT;
...
static inline LPS22DFTask* LPS22DFTaskGetOwnerFromISensorIF(ISensor_t *p_if)
{
  ...
  p_if_owner = (LPS22DFTask*) ((uint32_t) p_if - offsetof(LPS22DFTask, press_sensor_if));
