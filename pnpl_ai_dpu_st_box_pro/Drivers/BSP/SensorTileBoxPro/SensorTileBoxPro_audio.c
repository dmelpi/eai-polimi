...
//DMA_HandleTypeDef                       AMic_OnBoard_DmaHandle;
//MDF_DmaConfigTypeDef                    AMic_OnBoard_DmaConfig;
...
//int16_t AMIC_Buffer[DEFAULT_AUDIO_IN_BUFFER_SIZE];
...
//        }
//        
//        if(AudioInit->SampleRate == AUDIO_FREQUENCY_192K)
//        {
//          /** ADC Clk: 768kHz --> Decimation 4 = 192kHz
//           *  Sinc5, d=4 --> 22bit --> gain 2bit --> 8 bit shift obtained with DMA MsbOnly = 16bit
//           *  Sinc4, d=4 --> 20bit --> gain 4bit --> 8 bit shift obtained with DMA MsbOnly = 16bit
//           *  Gain it's expressed in 3dB steps --> half a bit
//           */
//          AMic_OnBoard_MdfFilterConfig.CicMode = MDF_ONE_FILTER_SINC5;
//          AMic_OnBoard_MdfFilterConfig.Gain = 2*2;
////          AMic_OnBoard_MdfFilterConfig.CicMode = MDF_ONE_FILTER_SINC4;
////          AMic_OnBoard_MdfFilterConfig.Gain = 4*2;
//          
//          AMic_OnBoard_MdfFilterConfig.DecimationRatio = 4;
//          
//          /** Offset to obtain signed data is applied in ADC, no need here.
//              In case, it needs to be half of output bits */
//          AMic_OnBoard_MdfFilterConfig.Offset = 0; 
////          AMic_OnBoard_MdfFilterConfig.Offset = 2097152; //(2^21), half of 2^22 for Sinc5
////          AMic_OnBoard_MdfFilterConfig.Offset = 524288;  //(2^19), half of 2^20 for Sinc4
//        }
//        else if(AudioInit->SampleRate == AUDIO_FREQUENCY_48K)
//        {
//          /** ADC Clk: 768kHz --> Decimation 16 = 48kHz
//           *  Sinc3,    d=16 --> 24bit --> gain 0bit --> 8 bit shift obtained with DMA MsbOnly = 16bit
//           *  FastSinc, d=16 --> 21bit --> gain 3bit --> 8 bit shift obtained with DMA MsbOnly = 16bit
//           *  Gain it's expressed in 3dB steps --> half a bit
//           */
//          AMic_OnBoard_MdfFilterConfig.CicMode = MDF_TWO_FILTERS_MCIC_SINC3;
//          AMic_OnBoard_MdfFilterConfig.Gain = 0;
////          AMic_OnBoard_MdfFilterConfig.CicMode = MDF_TWO_FILTERS_MCIC_FASTSINC;
////          AMic_OnBoard_MdfFilterConfig.Gain = 3*2;
//          
//          AMic_OnBoard_MdfFilterConfig.DecimationRatio = 16;
//          
//          /** Offset to obtain signed data is applied in ADC, no need here.
//              In case, it needs to be half of output bits */
//          AMic_OnBoard_MdfFilterConfig.Offset = 0; 
////          AMic_OnBoard_MdfFilterConfig.Offset = 8388608; //(2^23), half of 2^24 for Sinc3
////          AMic_OnBoard_MdfFilterConfig.Offset = 1048576; //(2^20), half of 2^21 for FastSinc
//        }
//        else
//        {
//          return BSP_ERROR_WRONG_PARAM;
...
//      }
      ...
      if ((AudioInCtx[Instance].Device & ONBOARD_DIGITAL_MIC_MASK) != 0U)
      {
        ...
        DMic_OnBoard_MDFFilter.Init.CommonParam.OutputClock.Divider            = 5U;
        ...
        DMic_OnBoard_MdfFilterConfig.CicMode                            = MDF_ONE_FILTER_SINC4;
        DMic_OnBoard_MdfFilterConfig.DecimationRatio                    = 64U;
        DMic_OnBoard_MdfFilterConfig.Offset                             = 0;
        DMic_OnBoard_MdfFilterConfig.Gain                               = 0;
        DMic_OnBoard_MdfFilterConfig.ReshapeFilter.Activation           = DISABLE;
        ...
        DMic_OnBoard_MdfFilterConfig.Integrator.Value                   = 4U;
        DMic_OnBoard_MdfFilterConfig.Integrator.OutputDivision          = MDF_INTEGRATOR_OUTPUT_NO_DIV;
        DMic_OnBoard_MdfFilterConfig.SoundActivity.Activation           = DISABLE;
        DMic_OnBoard_MdfFilterConfig.SoundActivity.Mode                 = MDF_SAD_AMBIENT_NOISE_DETECTOR;
        DMic_OnBoard_MdfFilterConfig.SoundActivity.FrameSize            = MDF_SAD_8_PCM_SAMPLES;
        DMic_OnBoard_MdfFilterConfig.SoundActivity.Hysteresis           = DISABLE;
        DMic_OnBoard_MdfFilterConfig.SoundActivity.SoundTriggerEvent    = MDF_SAD_ENTER_DETECT;
        DMic_OnBoard_MdfFilterConfig.SoundActivity.DataMemoryTransfer   = MDF_SAD_NO_MEMORY_TRANSFER;
        DMic_OnBoard_MdfFilterConfig.SoundActivity.MinNoiseLevel        = 0U;
        DMic_OnBoard_MdfFilterConfig.SoundActivity.HangoverWindow       = MDF_SAD_HANGOVER_4_FRAMES;
        DMic_OnBoard_MdfFilterConfig.SoundActivity.LearningFrames       = MDF_SAD_LEARNING_2_FRAMES;
        DMic_OnBoard_MdfFilterConfig.SoundActivity.AmbientNoiseSlope    = 0U;
        DMic_OnBoard_MdfFilterConfig.SoundActivity.SignalNoiseThreshold = MDF_SAD_SIGNAL_NOISE_3_5DB;
        DMic_OnBoard_MdfFilterConfig.SoundActivity.SoundLevelInterrupt  = DISABLE;
...
//      {     
//        /***** TO BE CHECKED: usare lo stesso ADC del BC e quindi implementare deinit in SensorTileBoxPro.c? *******/
...
//        AudioInCtx[0].HP_Filters[1].Z = (AMIC_Buffer[j + ((AudioInCtx[0].SampleRate / 1000U)* N_MS_PER_INTERRUPT)]) * (int32_t)(AudioInCtx[0].Volume);
...
//        AudioInCtx[0].pBuff[(AudioInCtx[0].ChannelsNbr*j)+1U] = (uint16_t) SaturaLH(AudioInCtx[0].HP_Filters[1].oldOut, -32760, 32760);          
      }
    }
...
//        AudioInCtx[0].HP_Filters[1].oldOut = (0xFC * (AudioInCtx[0].HP_Filters[1].oldOut + AudioInCtx[0].HP_Filters[1].Z - AudioInCtx[0].HP_Filters[1].oldIn)) / 256;
//        AudioInCtx[0].HP_Filters[1].oldIn = AudioInCtx[0].HP_Filters[1].Z;
////        AudioInCtx[0].pBuff[AudioInCtx[0].ChannelsNbr*j] = (uint16_t) SaturaLH(AudioInCtx[0].HP_Filters[1].oldOut, -32760, 32760);
  ...
  else
  {
    if ((AudioInCtx[0].Device & ONBOARD_DIGITAL_MIC_MASK) != 0U)
    {
      for (j = 0U; j < ((AudioInCtx[0].SampleRate / 1000U)* N_MS_PER_INTERRUPT); j++)
      {
        ...
        AudioInCtx[0].pBuff[AudioInCtx[0].ChannelsNbr * (j)] = (uint16_t) SaturaLH(AudioInCtx[0].HP_Filters[0].oldOut, -32760, 32760);
        /*AMIC_ONBOARD_CHANNEL*/
//        AudioInCtx[0].HP_Filters[1].Z = AMIC_Buffer[j] *  (int32_t)(AudioInCtx[0].Volume);
//        AudioInCtx[0].HP_Filters[1].oldOut = (0xFC * (AudioInCtx[0].HP_Filters[1].oldOut + AudioInCtx[0].HP_Filters[1].Z - AudioInCtx[0].HP_Filters[1].oldIn)) / 256;
//        AudioInCtx[0].HP_Filters[1].oldIn = AudioInCtx[0].HP_Filters[1].Z;
//        AudioInCtx[0].pBuff[(AudioInCtx[0].ChannelsNbr*j)+1U] = (uint16_t) SaturaLH(AudioInCtx[0].HP_Filters[1].oldOut, -32760, 32760);
      }
...
void HAL_MDF_MspInit(MDF_HandleTypeDef *hmdf)
{
  ...
  {
    /** Initializes the peripherals clock  to be at 3.072MHz to mic and 15.36Mhz to MDF*/
    RCC_PeriphCLKInitTypeDef PeriphClkInit = {0};
    PeriphClkInit.PeriphClockSelection = RCC_PERIPHCLK_ADF1;
    PeriphClkInit.Adf1ClockSelection = RCC_ADF1CLKSOURCE_PLL3;
    PeriphClkInit.PLL3.PLL3Source = RCC_PLLSOURCE_HSE;
    PeriphClkInit.PLL3.PLL3M = 2; //1;               /// mod nico era 2
    PeriphClkInit.PLL3.PLL3N = 48;               /// mod nico era 4   24   
    PeriphClkInit.PLL3.PLL3P = 2; //48;      /// uso Q, non cambia nulla qui
    PeriphClkInit.PLL3.PLL3Q = 25; //50;               /// mod nico era 128  125 
    PeriphClkInit.PLL3.PLL3R = 2;        /// uso Q, non cambia nulla qui
...
//    
//    /* Build NodeTx */
//    HAL_DMAEx_List_BuildNode(&dmaLinkNodeConfig, &AMic_OnBoard_DmaNode);
//    
//    /* Insert NodeTx to SAI queue */
//    HAL_DMAEx_List_InsertNode_Tail(&AMic_OnBoard_MdfQueue, &AMic_OnBoard_DmaNode);
//    
//    AMic_OnBoard_DmaHandle.Instance = GPDMA1_Channel2;
//    
//    /* Set queue circular mode for sai queue */
//    HAL_DMAEx_List_SetCircularMode(&AMic_OnBoard_MdfQueue);
//        
...
//    
//    /* DMA linked list init */
//    HAL_DMAEx_List_Init(&AMic_OnBoard_DmaHandle);
//    
//    /* Link SAI queue to DMA channel */
//    HAL_DMAEx_List_LinkQ(&AMic_OnBoard_DmaHandle, &AMic_OnBoard_MdfQueue);
//    
