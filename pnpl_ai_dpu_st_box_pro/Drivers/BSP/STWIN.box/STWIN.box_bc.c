...
#endif
{
  ...
  if(adcHandle->Instance==ADC4)
  {
    ...
    PeriphClkInit.AdcDacClockSelection = RCC_ADCDACCLKSOURCE_HSI; //RCC_ADCDACCLKSOURCE_SYSCLK;
...
void BC_Chg_Freq2Status(void)
{
  ...
  if(freq > (float) 0)
  {
    ...
    {
      if((freq < ((stbc02_ChgStateNameAndFreq[ChgState].freq + stbc02_ChgStateNameAndFreq[(uint8_t) ChgState + 1U].freq) / (float) 2)))
      {
        stbc02_ChgState = ChgState;
        break;
      }
    }
    if((freq > ((stbc02_ChgStateNameAndFreq[(uint8_t) ChgState - 1U].freq + stbc02_ChgStateNameAndFreq[ChgState].freq) / (float) 2)))
    {
      stbc02_ChgState = ChgState;
    }
  }
