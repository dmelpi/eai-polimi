...
#define INIT_IS_KIND_OF_AMTEX(pTask)            ((pTask)->m_xStatus.nReserved == 1U ? true : false)
...
static void InitTaskRun(void *pParams) {
   ...
   * The function SysLoadApplicationContext() is weak and it is redefined at application level.
   * This MISRA message is not reliable.
   */
...
static uint16_t InitTaskDoEnterPowerModeForPMClass(ApplicationContext *pxContext, EPMClass ePowerModeClass, const EPowerMode eActivePowerMode, const EPowerMode eNewPowerMode) {
  ...
  do {
    ...
    for (; pTask!=NULL; pTask=ACGetNextTask(pxContext, pTask)) {
      ...
      if (INIT_IS_KIND_OF_AMTEX(pTask))
      {
        eTaskPMClass = AMTExGetPMClass((AManagedTaskEx*)pTask);
      }
      else
      {
        eTaskPMClass = E_PM_CLASS_0;
      }
