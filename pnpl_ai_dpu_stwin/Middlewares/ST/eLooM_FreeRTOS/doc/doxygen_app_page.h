 ...
 *
 * The application implements two managed tasks, one to blink the LED with a regular interval (::AppTask), and another to put the system in low power mode and wake-up it when the user
 ...
 *
 * \subsubsection eloom_app_AppTask_SL Service Layer
 * This demo, due to its simplicity, does not provide any service in this layer.
 *
 * \subsubsection eloom_app_AppTask_AL Application Layer
 * The ::AppTask extends the ::AManagedTask interface. This is because the task feature is well modeled by this base class a control loop that implements a short step
 * in order to keep the system responsive.
 *
 * This managed task allocate the driver (NucleoDriver) in the AppTask_vtblHardwareInit() virtual function and it exports the API to share the driver object
 * with the other managed tasks - AppTaskGetDriver().
 *
 ...
 * This managed task uses the driver allocated by the ::AppTask. The link between the driver and the task is done in the SysOnStartApplication()
